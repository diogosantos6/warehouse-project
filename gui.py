...
class Window(tk.Tk):
    ...
    def show_solution_step(self):
        ...
        self.update_idletasks()
        self.after_id = self.after(0, self.show_solution_step)  # Delay
...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        ...
        for pair in self.agent.pairs:
            ...
            if state.matrix[pair.cell1.line][pair.cell1.column] == constants.FORKLIFT:
                state.line_forklift = pair.cell1.line
                state.column_forklift = pair.cell1.column
            else:
                state.line_forklift = pair.cell1.line
                state.column_forklift = pair.cell1.column - 1 if state.matrix[pair.cell1.line][pair.cell1.column - 1] == constants.EMPTY else pair.cell1.column + 1

            goal = copy.copy(pair.cell2)

            if state.matrix[pair.cell2.line][pair.cell2.column] != constants.EXIT:
                goal.column = pair.cell2.column - 1 if state.matrix[pair.cell2.line][pair.cell2.column - 1] == constants.EMPTY else pair.cell2.column + 1

            problem = WarehouseProblemSearch(state, goal)
            solution = self.agent.solve_problem(problem)

            pair.value = solution.cost
            pair.path_cells = solution.path_cells
...
class SolutionRunner(threading.Thread):
    ...
    def run(self):
        self.thread_running = True
        exit_column = self.state.column_exit
        exit_line = self.state.line_exit
        forklift_path, steps = self.best_in_run.obtain_all_path()
        genome, products = self.best_in_run.obtain_genome_and_products()
        ...
        for step in range(steps - 1):
            ...
            for j in range(len(forklift_path)):
                ...
                # Ver se é um produto, e se é do forklift
                if new_cell.column + 1 < self.state.columns:
                    if self.state.matrix[new_cell.line][new_cell.column + 1] == constants.PRODUCT\
                            and self.is_product_mine(genome, Cell(new_cell.line, new_cell.column + 1), j+1, products):
            ...
            # Meter a saída a azul quando o forklift lá estiver
            self.state.matrix[exit_line][exit_column] = constants.EXIT

            self.gui.queue.put((copy.deepcopy(self.state), step, False))
        self.gui.queue.put((None, steps, True))  # Done

    def is_product_mine(self, genome, cell, forkliftnumber, products) -> bool:
        countforklifts = 1
        for i in range(len(genome)):
            if genome[i] <= len(products):
                if products[genome[i] - 1] == cell:
                    if countforklifts == forkliftnumber:
                        return True
                    else:
                        return False
            else:
